...
export function composeSystemPrompt({
  ...
  // The shared renderer above owns these blocks whenever DESIGN.md is
  // available. Keep the upstream fallback for partial/imported packages that
  // expose component or pull-layer context without an active DESIGN.md body.
  if (
  ...
  if (
    !activeDesignSystemBody
    && designSystemPullIndex
    && designSystemPullIndex.trim().length > 0
  ) {
    parts.push(
